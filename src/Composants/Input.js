    ...
    return (
      <div className={"col-3 input-effect " + props.containerClass}>
        <input
          ...
          onBlur={(event) => {
            manageLabelOut(event);
          }}
        />
        <label id={"inputLabel" + props.id}>{props.label}</label>
        <FontAwesomeIcon
          className="pwdToggleVis"
          title={
            passwordShown ? "Cacher le mot de passe" : "Montrer le mot de passe"
          }
          onClick={() => setPasswordShown(!passwordShown)}
          icon={passwordShown ? "fa-solid  fa-eye-slash" : "fa-solid  fa-eye"}
        />
    ...
    return (
      <div className={"col-3 input-effect " + props.containerClass}>
        <input
          ...
          className={
            "input " +
            ((props.parDef && "has-content") || "") +
            props.inputClass
          }
          type={props.fieldType}
          placeholder=""
          defaultValue={props.parDef || ""}
          onFocus={(event) => {
            manageLabelIn(event);
          }}
          onBlur={(event) => {
