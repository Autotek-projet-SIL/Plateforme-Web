...
function ProfileATC(props) {
    ...
    if (newPdp.length===0) {
    ...
    else if (!newPdp[0].type.includes("image"))
      {
        ...
        try {
          ...
          http.put("/gestionprofils/modifier_atc/modifier_photo/"+viewedUser.id_atc,
          {"token" : decryptData(window.localStorage.getItem("currTok")),
          "id_sender":decryptData(window.localStorage.getItem("curruId")),
          "photo_atc": img
  ...
  async function  modifierInfos ()
  {
    ...
    let prenom = formatString(document.querySelector("#inputatcModifPrenom").value);
    ...
    if ((nom==="")||(prenom==="")||(numTel===""))
    {
      ...
      (nom==="") && document.querySelector("#inputatcModifNom").classList.add("input-error");
      (prenom==="") && document.querySelector("#inputatcModifPrenom").classList.add("input-error");
      (numTel==="") && document.querySelector("#inputatcModifTlfn").classList.add("input-error");
    }
    else{
      http.put("/gestionprofils/modifier_atc/"+viewedUser.id_atc,
      {"token" : decryptData(window.localStorage.getItem("currTok")),
      "id_sender":decryptData(window.localStorage.getItem("curruId")),
      "nom" : nom,
      "prenom": prenom,
    ...
    return (
      ...
      <div id="monCompteCard">
          ...
          <div id="modifInfosProfile">
            <Input label="Votre nom" inputClass="" containerClass="modifAtcInput" id="atcModifNom" fieldType="text" parDef={viewedUser.nom}/>
            <Input label="Votre prénom" inputClass="" containerClass="modifAtcInput" id="atcModifPrenom" fieldType="text" parDef={viewedUser.prenom}/>
            <Input label="Votre numéro de téléphone" inputClass="" containerClass="modifAtcInput" id="atcModifTlfn" fieldType="tel"parDef={viewedUser.numero_telephone}/>
          </div>
