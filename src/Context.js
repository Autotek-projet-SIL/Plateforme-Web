  ...
  const login = async (autUser, typeUser) => {
    ...
    try {
      ...
      if (typeUser === "atc") {
        ...
        http
          .get(`authentification_web/atc_connexion/${autUser.email}`, {
            headers: {
          ...
          .then((jResponse) => {
            ...
            if (jResponse.data.length === 0) {
              ...
              window.localStorage.setItem(
                "currEm",
                encryptData(jResponse.data[0].email)
              ); // email de l'utilisateur
              window.localStorage.setItem(
                "curruId",
                encryptData(response.user.uid)
              ); // ID de l'utilisateur
              window.localStorage.setItem(
                "currTok",
                encryptData(response.user.accessToken)
              ); // Access token
          ...
          .catch((err) => {
            ...
            if (err.message === "Network Error") {
              alert.error(
                "Une erreur est survenue, veuillez vérifier votre connexion ou réessayer utérieurement."
              );
            } else {
              alert.error("Identifiants incorrects");
            }
        ...
        http
          .get(`authentification_web/decideur_connexion/${autUser.email}`, {
            headers: {
              token: response.user.accessToken,
              id_sender: response.user.uid,
            },
  ...
  const refreshUser = async (type) => {
    ...
    getTheAuth().onIdTokenChanged(async (currentUser) => {
      try {
        ...
        window.localStorage.setItem(
          "currTok",
          encryptData(await currentUser.getIdToken())
        );
        window.localStorage.setItem("curruId", encryptData(currentUser.uid));
        if (type === "atc") {
          http
            .get(
              `authentification_web/atc_connexion/${decryptData(
                window.localStorage.getItem("currEm")
              )}`,
              {
                headers: {
            ...
            .then((jResponse) => {
              ...
              if (jResponse.data.length === 0) {
                alert.error(
                  "Il semble que vous n'avez pas accès à ce contenu."
                );
                loggingOut();
              } else {
          ...
          http
            .get(
              `authentification_web/decideur_connexion/${decryptData(
                window.localStorage.getItem("currEm")
              )}`,
              {
                headers: {
